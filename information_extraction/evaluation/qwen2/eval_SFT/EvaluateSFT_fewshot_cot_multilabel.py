...
def extract_answer(answer, num_answers=2, stop_at_line_break=False, **kwargs):
    ...
    if answer != '':
        ...
        if len(result) == 0:
            if args["variable"] == "type_ica":
                result = ['E']
            elif args["variable"] == "cardiopathie_causale":
                result = ['D']
            elif args["variable"] == "facteur_decl":
                result = ['F']
    else:
        if args["variable"] == "type_ica":
            result = ['E']
...
for c in dataset[0]['classes']:
    ...
    for r in all_results:
        if c in r['correct_letter']:
            true_labels.append(c)
        else:
            true_labels.append('N')

        if c in r['response']:
            true_predictions.append(c)
        else:
            true_predictions.append('N')
